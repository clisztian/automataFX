...
public class TimeSeriesRegressionExample {
	...
	public static void main(String[] args) throws IllegalArgumentException, IllegalAccessException {
		...
		int nClasses = 1; //regression
		
		
		date_formatter = DateTimeFormat.forPattern(date_time_format);
		
		int total_samples = 3500;
		int out_sample = 200;
		ArrayList<AnomalySeriesObservation> in_sample = TimeSeriesRegressionExample.sampleStochasticData(0, total_samples);
		...
		for(int i = n_samples; i < in_sample.size()-1; i++) {
			...
			float pred = automata.predict_regression(in_sample.get(i));
			//System.out.println(i + " " + in_sample.get(i).value() + " " + in_sample.get(i+1).value() + " " + pred);
			predictions.add(pred);
			error_sum += (pred - in_sample.get(i+1).value())*(pred - in_sample.get(i+1).value());
